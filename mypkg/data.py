...
def load_toy_dataset():
    ...
    return trn_X, trn_Y, tst_X, tst_Y, labels_names
...
class Dataset(Configurable):
    ...
    def load(self):
        attr_names = None
        if self.name == "toy":
            trn_X, trn_Y, tst_X, tst_Y, labels_names = load_toy_dataset()
        ...
        if self.attr_names is not None:
            self.attr2id = {name: i for i, name in enumerate(self.attr_names)}
            self.id2attr = {id_: attr for attr, id_ in self.attr2id.items()}
        # label frequency
